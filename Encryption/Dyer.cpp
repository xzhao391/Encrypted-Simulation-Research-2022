...
extern "C"
{
    ...
    __declspec(dllexport) void control(double xm, double dxm, double ddxm, double xs, double dxs,
        ...
        auto read_prime_from_file = [&](int bit_length) {
            std::string FILE_NAME = "D:\\Enc_3\\Dyer_Simulink\\primes.txt";
            int NO_PRIME = 0;
    ...
    //__declspec(dllexport) PKey keygen(int bit_length, int rho, int rho_) {
    //    wide_uint_t p = get_prime(bit_length);
    //    int nu = rho_ - rho;
    //    wide_uint_t kappa = get_prime(nu);
    //    Dyer::PKey key = { kappa, p };
    //    return key;
    //}

    //wide_uint_t Dyer::get_prime(int bit_length) {
    //    wide_uint_t p = read_prime_from_file(bit_length);
    //    return p;
    //}

    //wide_uint_t Dyer::read_prime_from_file(int bit_length)
    //{
    //    std::string FILE_NAME = "D:\\Enc_3\\Dyer_Simulink\\primes.txt";
    //    int NO_PRIME = 0;
    ...
    //wide_uint_t Dyer::pgen(int bit_length, int rho_, wide_uint_t p) {
    //    int eta = std::floor(std::pow(bit_length, 2) / rho_) - bit_length;
    //    wide_uint_t q = get_prime(eta);
    //    wide_uint_t modulus = p * q;
    ...
    //wide_uint_t Dyer::encode(wide_double_t x, wide_double_t delta) {
    //    return wide_uint_t(mp::floor(x / delta + .5));
    //}


    //wide_double_t Dyer::decode(int m, wide_double_t delta) {
    //    wide_double_t x = m * delta;
    //    return x;
    //}

    //wide_uint_t Dyer::get_rand(wide_uint_t min, wide_uint_t max) {
    //    static rng::mt11213b rng(clock());
    //    rng::uniform_int_distribution<wide_uint_t> distribution(min, max);
    ...
    //cipher_text Dyer::encrypt(wide_uint_t m, PKey key, wide_uint_t modulus) {
    //    wide_uint_t q(mp::floor(
    //        (modulus / key.p).convert_to<wide_double_t>()
    //    ));
    //    //    wide_int_t r = std::rand() % (q - 1) + 1;
    //    wide_uint_t r = get_rand(0, q - 1);
    //    //    wide_int_t s = std::rand() % kappa;
    //    wide_uint_t s = get_rand(0, key.kappa - 1);
    ...
    //wide_int_t Dyer::min_residue(const wide_int_t& a, const wide_int_t& m) {
    //    wide_int_t b = a % m;
    //    wide_int_t c = b - m;
    //    return b >= abs(c) ? c : b;
    //}

    //int Dyer::decrypt(cipher_text c, PKey key) {  // MIGHT NEED `min_residue"
    //    //return static_cast<int>(min_residue(min_residue(c, key.p),key.kappa));
    //    return static_cast<int>(min_residue(c % key.p, key.kappa));
    //}


    //cipher_text Dyer::enc(wide_double_t x, PKey key, wide_uint_t modulus, wide_double_t delta) {
    //    return encrypt(encode(x, delta), key, modulus);
    //}


    //wide_double_t Dyer::dec(wide_int_t c, PKey key, wide_double_t delta) {
    //    return decode(decrypt(c, key), delta);
    //}

    //cipher_text Dyer::add(cipher_text c1, cipher_text c2, wide_uint_t modulus) {
    //    return (c1 + c2) % modulus;
    //}

    //cipher_text Dyer::mul(cipher_text c1, cipher_text c2, wide_uint_t modulus) {
    //    return (c1 * c2) % modulus;
    //}
//}